...
describe('গবেষকের কিউ — আপলোড ও লাইভের অপেক্ষায়', () => {
  ...
  async function markDoneAt(jobNumber: number, iso: string): Promise<void> {
    await h.prisma.designTarget.update({
      where: { jobNumber },
  ...
  it('⚠️⚠️ কাটা-তারিখের আগের "শেষ" কিউতে ওঠে না', async () => {
    ...
    await markDoneAt(JOB_NUMBER_START, '2026-08-22T10:00:00+06:00');
    // ২৩ আগস্ট — সীমার দিনেই, অর্থাৎ গোনা হবে
    await markDoneAt(JOB_NUMBER_START + 1, '2026-08-23T10:00:00+06:00');

    const page = await targetsOf().list({ stage: 'to_upload' });

    expect(page.total).toBe(1);
    expect(page.rows[0].jobNumber).toBe(JOB_NUMBER_START + 1);
  });
  ...
  it('চিপের সংখ্যা আর তালিকার সংখ্যা হুবহু এক', async () => {
    await markDoneAt(JOB_NUMBER_START, '2026-08-23T10:00:00+06:00');
    await markDoneAt(JOB_NUMBER_START + 1, '2026-08-24T10:00:00+06:00');
    await markDoneAt(JOB_NUMBER_START + 2, '2026-08-22T10:00:00+06:00');
  ...
  it('আপলোড হয়ে গেলে সারিটা প্রথম কিউ ছেড়ে দ্বিতীয়টায় যায়', async () => {
    ...
    const row = await h.prisma.designTarget.findUniqueOrThrow({
      where: { jobNumber: JOB_NUMBER_START },
    });
  ...
  it('লাইভ-কিউতে কাটা-তারিখ খাটে না', async () => {
    await markDoneAt(JOB_NUMBER_START, '2025-01-10T10:00:00+06:00');
    const row = await h.prisma.designTarget.findUniqueOrThrow({
      where: { jobNumber: JOB_NUMBER_START },
    });
    await targetsOf().markUploaded(row.id, new Date());
