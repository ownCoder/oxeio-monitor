...
| ⭐ **`ssh: Connection timed out`** | সার্ভার সম্ভবত দিব্যি চলছে — শুধু ২২ পোর্টের পথ বন্ধ, নিচে § ১২.৪ |
...
| ফল | মানে | করণীয় |
|---|---|---|
| `False` | **আপনার নেটওয়ার্ক** বাইরের ২২ আটকাচ্ছে (অফিস/ISP-তে খুব সাধারণ) | ফোনের হটস্পটে জুড়ুন — সাথে সাথেই কাজ করবে |
| `True` | বাধাটা **সার্ভারে** — ufw বা sshd | ওয়েব কনসোল লাগবে, নিচে দেখুন |

**ওয়েব কনসোল — যেটা কোনো পোর্টের উপর নির্ভর করে না**

হোস্টিং প্যানেলে **Console / VNC / KVM** নামে থাকে। ব্রাউজারেই টার্মিনাল
খোলে, তাই ফায়ারওয়াল যা-ই হোক ঢোকা যায়। ⭐ ২২ পোর্ট সার্ভারেই বন্ধ হয়ে
গেলে **এটাই একমাত্র পথ** — তাই প্যানেলে ওটা কোথায়, সেটা আজই দেখে রাখুন,
দরকারের দিন খুঁজতে হবে না।

ঢুকে কারণ দেখা ও সারানো:

```bash
ufw status && systemctl status ssh --no-pager
```

```bash
ufw allow OpenSSH && systemctl restart ssh
```
