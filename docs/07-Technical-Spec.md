...
| **Settings** | Work policy (মাসিক টার্গেট, সাপ্তাহিক ছুটির দিন, ক্যাপচার উইন্ডো), screenshot interval, idle threshold, retention, app categories, ছুটির ক্যালেন্ডার, স্টাফ ম্যানেজমেন্ট। ⭐ ম্যানেজার তিনটে ট্যাব পান — Staff · Categories · **Holidays** *(১৫ আগস্ট, § ৪.৩)*; তাঁর কাছে work policy-র অংশটা দেখানোই হয় না, তাই ট্যাবের নামও তাঁর জন্য আলাদা। ⚠️⚠️ **Devices ট্যাব ইচ্ছাকৃতভাবে নেই** — মালিকের কথায় *"ami Devices ei option tai chai na"*; বন্ধ এজেন্ট ফেরানোর কাজটা এখন Staff সারিতেই ("Turn agent on"), অর্থাৎ মেশিন নয়, **মানুষ ধরে সাজানো** |
...
- [ ] **VPS তৈরি** ([ADR-026](05-Options-Decisions.md)) — ⭐ `deploy/vps-setup.sh hub.oxeio.com` **এক কমান্ডেই** Docker · ফায়ারওয়াল · গোপন মান · DNS যাচাই · স্ট্যাক করে দেয়। ⚠️ আগে DNS (`A` রেকর্ড) আর GitHub deploy key — দুটোই লাগে
- [ ] ⚠️⚠️ `certbot renew --reuse-key` — নইলে ৯০ দিন পর SPKI বদলে **১৫টা এজেন্ট একসাথে সংযোগ হারাবে** (অথবা `SERVERPIN` না দিয়ে পিনিং বন্ধ রাখুন)
...
> ⚠️ **কোড দিয়ে সারানো যায় না, এমন যা বাকি** *(১২ আগস্ট হালনাগাদ)*:
> G39 অফসাইট ব্যাকআপের গন্তব্য · টেলিগ্রামে আসল ডেলিভারি (মালিকের bot
> token লাগবে) · policy টেমপ্লেটের ৩৬টা ফাঁকা ঘর ও আইনজীবী ·
> স্টাফের `joined_on` (O13)।
>
> ✅ **যা আর বাকি নেই:** I01 সার্ট পিনিং **কোডে বসেছে**
> ([§ ৩ত](09-Build-Log.md)) · code signing **কিনতে হবে না** — নিজে সই
> করা হবে ([ADR-014](05-Options-Decisions.md)); ⚠️ স্ক্রিপ্ট তিনটে এখনো
> লেখা হয়নি, কিন্তু সেটা কোডের কাজ, সিদ্ধান্তের নয়।
