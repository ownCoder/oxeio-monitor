...
CREATE TABLE agent_versions (
  ...
  -- ⭐ ধাপ বাড়ে হাতে (owner) **অথবা** নিজে থেকে — শর্ত: একটা আসল মেশিন
  --    ৬ঘ ধরে বিল্ডটা চালিয়ে শেষ ৯০ মিনিটে সাড়া দিয়েছে
  --    (G141 · devices.agent_version_since · ROLLOUT_SOAK_HOURS/ROLLOUT_FRESH_MINUTES)
...
ততদিন সত্যের উৎস `schema.prisma`।
...
work_date  =  started_at-এর Asia/Dhaka তারিখ (সব টেবিলে একই নিয়ম) — `workDateOf()`
```

> ⚠️⚠️ **`work_date` একটা লেবেল, মুহূর্ত নয়।** `workDateOf()` ঢাকার দিনটাকে
> **UTC-মধ্যরাত** হিসেবে লিখে রাখে, অথচ ওই দিনের আসল ঢাকা-মধ্যরাত শুরু হয়
> তার **৬ ঘণ্টা আগে**। `DATE` কলামে বসলে পার্থক্যটা চোখে পড়ে না, কিন্তু
> `completed_at`-এর মতো `timestamptz` কলাম দিয়ে দিন-ধরে ছাঁকতে গেলে সীমানা
> **হাতে গুনতে হয়** (`DHAKA_OFFSET_MS = ৬ × ৩৬০০০০০`)।
>
> ⚠️ লেবেলটাকে সোজা সীমানা হিসেবে বসালে জানালাটা ৬ ঘণ্টা দেরিতে সরে যায় —
> প্রথম দিনের মধ্যরাত-থেকে-ভোরের কাজ বাদ পড়ে। আর UTC-র দিন ধরে বালতি করলে
> ঢাকার রাত ১২টা–ভোর ৬টার কাজ **আগের দিনের** ঘরে পড়ে। দুটোই নীরব
> ([09 § ৩ঞ২৯](09-Build-Log.md))। ⭐ `workDateOf()` গোটা সিস্টেমে *"কোন দিন"*
> ঠিক করার **একমাত্র** জায়গা — দ্বিতীয় সংজ্ঞা মানেই একদিন দুই পাতা দুই
> সংখ্যা বলা।
...
> ⭐⭐ **`workdays_elapsed = 0` একটা আলাদা অবস্থা, ০-ঘাটতি নয়** *(G111,
> ৫ সেপ্টেম্বর ২০২৬)*।
>
> ⚠️⚠️ জানালাটা খালি হলে (আজই প্রথম দিন, বা এজেন্ট সবে বসেছে) প্রত্যাশা ০,
> তাই `pace_sec`-ও ০ — আর পর্দায় সেটা **হুবহু টার্গেট পূরণ করা মানুষের মতো**
> দেখাত। দুটো সম্পূর্ণ বিপরীত অবস্থা একই চেহারা পেলে সংখ্যাটা আর কোনো
> প্রশ্নের উত্তর দেয় না।
>
> ⭐ তাই এটা একটা **অবস্থা**, সংখ্যা নয়: `isObserved({ workdaysElapsed })`
> (`summary.math.ts`), আর চারটে পর্দাই ওটাই পড়ে — tray/`/me` (`observed`),
> Live Board (`observedStaff` / `notObservedStaff`), Monthly
> (`ReportMeta.observed`), আর এজেন্টের tray (`AgentStatus.PaceObserved`)।
>
> ⚠️ **দলগত যোগফলে না-দেখা মানুষের টার্গেট যোগ করা হয় না** — তাতে বোর্ড
> এমন ঘাটতির দাবি করত যেটা কেউ করেনি। সংখ্যাটা **বাদ দেওয়া হয় না, বলা হয়**:
> কার্ডে পাশে *"N not counted yet"*।
>
> ⚠️ পর্দা যেন `pace_sec === 0` দেখে নিজে সিদ্ধান্ত না নেয় — এজেন্টে ওই
> ভুলটা বিশেষভাবে ক্ষতিকর: `Pace = null` মানে "সার্ভার বলেনি", আর তখন tray
> **নিজের আনুমানিক হিসাবে** ফিরে যায় (মাসের ১ তারিখ থেকে গোনে), অর্থাৎ ঠিক
> ওই না-দেখা দিনগুলোকেই ঘাটতি বলত। ক্রমের নিয়মটা তাই খাঁটি ফাংশনে:
> `MonthlyPace.ViewFor()`।
...
> ⭐⭐ **tray/`/me`-ও এখন এই UNION-ই পড়ে — `Σ duration_sec` নয়** *(G112,
> ৫ সেপ্টেম্বর ২০২৬)*।
>
> ⚠️⚠️ tray আগে ঘণ্টা গুনত `Σ activity_segments.duration_sec` দিয়ে, অর্থাৎ
> এজেন্টের **monotonic ঘড়ির কাঁচা যোগফল** — উপরের নিয়মটার ঠিক উল্টো।
> ফলে দুই ডিভাইসে একসাথে কাজ করলে সময়টা tray-তে **দুবার** গোনা হতো, আর
> কর্মী নিজের tray-তে ড্যাশবোর্ডের চেয়ে **বেশি ঘণ্টা** দেখতেন। ⭐ `device_overlap`
> অ্যালার্ট ঠিক ওই ফারাকটাই মাপে — সিস্টেম সংখ্যাটা জানত, শুধু tray জানত না।
>
> **এক সংজ্ঞা, দুটো উৎস:**
>
> ```
...
> ```
>
> ⚠️⚠️ সীমানাটা `work_date < today`, `<=` নয় — নইলে আজকের কাজ দুই উৎস
> থেকেই আসত। ⚠️ আজকের দিন rollup থেকে নেওয়া যায় না (১৫ মিনিট বাসি, আর
> tray-র কাজই "এখন কত হলো"), কিন্তু **সংজ্ঞাটা এক** — নইলে মধ্যরাতে
> সীমানা পেরোনোর সময় সংখ্যাটা নিজে থেকেই লাফাত।
>
> ⚠️ **দাম:** ঢাকার মধ্যরাত থেকে ০০:১৫ (day-close) পর্যন্ত গতকালের সারি
> চূড়ান্ত নয়, তাই tray কয়েক মিনিট কম দেখাতে পারে — কিন্তু ভুলটা তখন
> ড্যাশবোর্ডের সাথে **অভিন্ন**, আর সেটাই কাম্য।
...
| `monthCreditedSec` | ⭐ *নতুন* **(৬ সেপ্টেম্বর ২০২৬, G162)** — চলতি মাসের **গোনা** সেকেন্ড: `monthActiveSec` + সময়-সংশোধন। ⚠️⚠️ `monthActiveSec`-এর সাথে গুলিয়ে ফেলা যাবে না, আর দুটোই দরকার: টাইল ও tray দেখায় **কত কাজ হয়েছে** (`worked`), আর My data-র নিচের যোগফল দেখায় **কত গোনা হয়েছে** (`credited`)। আগে নিচের সংখ্যাটা ব্রাউজারে কষা হতো আর তিনটে আলাদা কারণে উপরের টাইলের সাথে মিলত না ([09 § ৩ঞ৩১](09-Build-Log.md))। ⚠️ `monthActiveSec` বদলানো হয়নি — ওটা এজেন্টের তারের চুক্তি |
...
| GET | `/live/pulse` | দিনের ছন্দ — ঘণ্টাভিত্তিক দলগত কার্ভ |
| GET | `/live/trend` | ⭐ শেষ ৭ দিনের দলগত প্রবণতা → `{ days: TrendDay[] }` *(ফিল্ড ↓)* |
| GET | `/employees/:id/hourly` | একজনের ঘণ্টাভিত্তিক কার্ভ |
...
| GET | `/screenshots/latest` | ⭐ *নতুন* **(৬ সেপ্টেম্বর ২০২৬, G159)** — আজকের দিনে **কর্মীপ্রতি সবচেয়ে নতুন** ছবিটা, Live Board ও Worklog-এর কার্ডের জন্য। ⚠️⚠️ আগে পর্দা এই প্রশ্নটার উত্তর গ্যালারির **শেষ এক-দুটো পাতা** টেনে এনে অনুমান করত, তাই যাঁর শেষ ছবিটা ওই জানালার বাইরে তাঁর কার্ডে উঠত *"No screenshot yet today"* — ২৫ আগস্ট OX-05-এর ১১৪টা ছবি থাকা সত্ত্বেও। ⚠️ **অডিটে কলপ্রতি একটাই সারি**, কর্মীপ্রতি নয় — নইলে বোর্ড খোলামাত্র ১২টা সারি লিখে I08-এর খাতাটা আবর্জনায় ভরে যেত। কর্মী নিজে ডাকলে কেবল নিজেরটা, গ্যালারির হুবহু একই নিয়ম |
...
| GET·PATCH·POST | `/deposits` · `/deposits/policy` · `/deposits/:employeeId/settle` | ⭐ **owner only** (R21) — সিকিউরিটি মানি। ⚠️ ম্যানেজারও নয়: জামানত সরাসরি বেতনের অংশ। ⭐⭐ খাতা **লিখে রাখা** হয়, গোনা হয় না — অঙ্ক বদলালে পুরোনো কিস্তি নড়ে না। ⚠️ `settle` দ্বিতীয়বার ডাকলে **৪০৯**; সিদ্ধান্ত মালিকের, সিস্টেম কেবল নোটিশের দিন গুনে সারিতে লেখে ([ADR-028](05-Options-Decisions.md))। ⭐⭐ **`/deposits/:id/months` ও `/deposits/:id/instalment`** *(৫ সেপ্টেম্বর ২০২৬)* — মালিকও এখন মাস-ধরে খাতা দেখেন (এতদিন কেবল কর্মীর `/me/deposit`-এ ছিল), আর ভুল অঙ্ক সংশোধন করতে পারেন। ⚠️ **এটা নিয়ম বদল নয়, ভুল সংশোধন** — তাই `reason` বাধ্যতামূলক, আর অডিটে আগের-পরের দুটো অঙ্কই যায়। ⚠️⚠️ **শূন্য বসানো যায় না** (DTO · সার্ভিস · `CHECK (amount_paisa > 0)` — তিন স্তরে): মকুব মানে ওই মাসে কিস্তি **নেই**, ৳০-এর কিস্তি **আছে**, আর দুটো এক করলে "কত মাস জমা হয়েছে" প্রশ্নের উত্তরই নষ্ট হয় (G145 · G146, [09 § ৩ঞ২৮](09-Build-Log.md))। ⚠️ বন্ধ মাসে সংশোধন **৪০৯** — ছুটির একই নিয়ম (R1) |
...
| POST | `/agent-versions/:version/stage` | ⭐ **owner only** — `canary`/`partial`/`all`/**`halted`**। খারাপ বিল্ড থামানোর একমাত্র ব্রেক (rollback নেই, G69)। ⚠️ **এটাই আর একমাত্র পথ নয়** *(৫ সেপ্টেম্বর ২০২৬, G141)*: `RolloutAdvanceJob` ঘণ্টায় একবার (`:07`) ধাপ বাড়ায় — কিন্তু কেবল তখনই, যখন একটা **আসল মেশিন** ওই বিল্ডটা `ROLLOUT_SOAK_HOURS` (৬) ধরে চালিয়ে **শেষ ৯০ মিনিটের মধ্যে সাড়া দিয়েছে** (`ROLLOUT_FRESH_MINUTES`)। ⚠️ ৯০, অ্যালার্টের ১০ নয় — প্রশ্ন দুটো আলাদা: অ্যালার্ট জানতে চায় *"এখনই কি কিছু ভাঙা"*, এই জব জানতে চায় *"মেশিনটা কি সাধারণভাবে চলছে"*; ১০ ধরলে একটা দুপুরের বিরতি বা রিবুটই ধাপ আটকে দিত। ⚠️⚠️ `halted` কখনো নিজে খোলে না, আর অডিটে আলাদা action (`agent_version.rollout_auto`), meta-তে `{to, from, onVersion, soakHours, freshMinutes}`। ✅ **মাঠে প্রথম আসল ধাপ ৫ সেপ্টেম্বর ১৭:০৭** — ০.৪.১১ `partial → all`, প্রমাণ ছিল একটাই মেশিন (পাইলট) |
...
**`TrendDay`** — `/live/trend`-এর প্রতিটা দিন:

| ফিল্ড | কী |
|---|---|
| `date` | ঢাকার কর্মদিবস, `YYYY-MM-DD` |
| `workedSec` · `targetSec` | ওই দিনে দলের গোনা ও প্রত্যাশিত সেকেন্ড। ⚠️ প্রত্যাশা আসে ক্যালেন্ডার থেকে (`trendDayExpectation()`), `daily_summary` সারি গুনে নয় (G118) |
| `expectedStaff` | ওই দিন কতজনের কাজ করার কথা ছিল — `targetSec`-এর হর |
| `tracked` | ⚠️⚠️ ওই দিন আদৌ দেখছিলাম কি না। `false` মানে *"কেউ কাজ করেনি"* **নয়** — ট্র্যাকিংই শুরু হয়নি। পর্দায় ড্যাশ ও ডটেড রেখা, `০` নয় (§ ২.১-খ) |
| `designsFinished` | ⭐⭐ *(৫ সেপ্টেম্বর ২০২৬)* ওই দিনে **শেষ হওয়া** ডিজাইনের সংখ্যা। ⚠️⚠️ উৎস `design_targets.completed_at`, **`design_credits` নয়** — ওটা কতগুলো ফাইল *খোলা* হয়েছে বলে ([ADR-036](05-Options-Decisions.md) · [ADR-037](05-Options-Decisions.md))। ⚠️ ওই টেবিলে `work_date` কলাম নেই, তাই বালতি `workDateOf()` ধরে আর কোয়েরির সীমানা হাতে গোনা (§ ২.১-ক) |
...
| স্ক্রিন | কী থাকবে |
|---|---|
| **Live Board** | দলের সারাংশ — KPI টাইল, ⭐ **Designs Finished** *(৫ সেপ্টেম্বর — শেষ ৭ দিন, গোটা দল; বার সবচেয়ে বড় দিনের সাপেক্ষে, ট্র্যাকিংয়ের আগের দিন ডটেড ও em-dash)*, তার নিচে **Where Today Went** ও অন্যান্য চার্ট, দলের টেবিল, খোলা অ্যালার্ট। ⚠️⚠️ **স্টাফের কার্ড এখানে নেই** *(১৭ আগস্ট)* — নিচের `Worklog` দেখুন |
...
| **Targets → Add target design** ⭐ | *(২২–২৩ আগস্ট)* গবেষক · ম্যানেজার · মালিক — একবারে ৫০০টা Amazon URL পেস্ট। ⚠️ বাতিল লাইনগুলো **লাইন নম্বর, লেখা ও কারণসহ** ফেরত আসে; ৫০০-র মধ্যে ৬টা বাদ পড়লে কোন ৬টা সেটা না জানলে ওগুলো আর সংগ্রহ করা হতো না |
| **Targets → All design targets** ⭐ | পুরো তালিকা — ছাঁকনি · **URL বা ASIN দিয়ে খোঁজা** · পাতা ভাগ (৫০/পাতা, টেবিলে ৩৯ হাজার সারি) · কে ও কবে · সম্পাদনা (To pool · Complete · Skip · Delete)। ⚠️⚠️ **ASIN বদলানোর পথ নেই** — ওটা সারিটার পরিচয়, আর ডুপ্লিকেট-প্রহরী ওটার উপরেই দাঁড়ানো |
| *(সাইডবারের ভাগ)* | ⭐ তিনটে ভাগ *(২৩ আগস্ট)* — **Targets** · **Team** · **Oversight**। ⚠️⚠️ ভাগের লেবেল কেবল **শুরু** চিহ্নিত করে, শেষ নয়; তাই "Targets"-এর পরে "Team" বসাতেই হয়েছে, নইলে Staff · Screenshots · Monthly সবই ওই ভাগের ভেতরে বলে মনে হতো। ⭐ ভাগের ভেতরের আইটেম ডানে সরে বসে, বাঁয়ে সরু রেখাসহ (`NavItem.child`) |
...
| **প্রতি ঘণ্টায় `:07`** | ⭐⭐ এজেন্ট-রোলআউট এগোনো — `canary → partial → all`। ⚠️ শর্ত **সময় নয়, প্রমাণ**: একটা আসল মেশিন বিল্ডটা ৬ঘ চালিয়ে **শেষ ৯০ মিনিটে** সাড়া দিয়েছে কি না। ⚠️⚠️ `halted` কখনো নিজে খোলে না (G141, [09 § ৩ঞ২৭](09-Build-Log.md))। ✅ প্রথম আসল ধাপ ৫ সেপ্টেম্বর ১৭:০৭ |
...
| ~~প্রতি ঘণ্টায়, ০৯:০০–১৯:০০~~ | ⛔ **তুলে দেওয়া হয়েছে** *(১৮ আগস্ট, [ADR-031](05-Options-Decisions.md))*। টেলিগ্রামে স্ন্যাপশট যেত — *"Working now: 9/13"*। এসেছিল রিয়েল-টাইম idle অ্যালার্টের **বিকল্প** হিসেবে (দিনে ৬০–১৮০-র বদলে ১১), কিন্তু হিসাবটা ভুল জায়গা থেকে শুরু হয়েছিল: ওই একই চ্যাটে `agent_down`ও যাচ্ছিল **দিনে ৩৯ বার**, তাই মোট দাঁড়াত ~৫০ — আর তার নিচে চাপা পড়ত সন্ধ্যা ৬:৩০-এর দৈনিক রিপোর্ট, যেটা মালিক আসলে চেয়েছিলেন। ⭐ "এখন কে কাজ করছে" Live Board-এ সবসময় আছে |
...
| ব্রুট-ফোর্স (I11) | ইমেইল+IP জোড়ায় ডিফল্টে ১০ বার ভুল → ২ মিনিট লক (429), সাথে একা IP-তে ৫০ বার (G116); প্রান্তে Caddy-তে `POST /auth/login` ৩০/মিনিট *(১৫ আগস্ট)*। ইন-মেমরি; সার্ভার রিস্টার্টে মুছে যায় |
